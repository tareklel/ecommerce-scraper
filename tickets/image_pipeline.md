...
metadata to a `image_validated` table — enabling downstream serving pipelines.
...
[1. run_image_pipeline.py]       ← new ECS orchestrator CLI
        ...
        ▼
[bronze/images/download_log/dt={dt}/data.jsonl.gz]   ← Athena partition
[bronze/images/download_log/meta/{dt}/_SUCCESS]       ← trigger marker (separate dir)
        │
        ▼
[2. ECS job: image_quality_checker]   ← triggered by _SUCCESS marker
        │  validates downloaded images (format, dimensions, not error page)
        │  appends enriched rows to image_validated Athena table
        ▼
[Athena: image_validated]              ← bronze layer, input to serving pipeline
```
...
5. Write per-image results to `bronze/images/download_log/dt={dt}/data.jsonl.gz`
6. Register new Athena partition in Glue
7. Write `_SUCCESS` or `_FAILED` to `bronze/images/download_log/meta/{dt}/`
...
- Drop `by-primary` — product→image mapping resolved through `image_download_log` and `image_validated`
...
**Responsibilities:**
1. Read `data.jsonl.gz` for the run from `bronze/images/download_log/dt={dt}/`
...
4. Append validated rows to `image_validated` Athena table partitioned by `dt`
5. Register new partition in Glue

**`image_validated`** — presence in this table means the image passed both download and quality
...
CREATE EXTERNAL TABLE image_download_log (
  site         STRING,
  primary_key  STRING,
  url          STRING,
  run_id       STRING,
...
STORED AS ... LOCATION 's3://price-comparison-bucket-eu-central-1/bronze/images/download_log/';

-- Written by image_quality_checker ECS job
CREATE EXTERNAL TABLE image_validated (
  site         STRING,
  primary_key  STRING,
  image_url    STRING,
  s3_blob_key  STRING,
...
PARTITIONED BY (dt STRING)
STORED AS ... LOCATION 's3://price-comparison-bucket-eu-central-1/bronze/images/validated/';
```
...
- **Glue:** tables (`image_download_log`, `image_validated`) owned and managed by scraper-pipeline — not created here
...
| `infra/terraform/glue.tf` | ~~Two new external tables~~ — removed, owned by scraper-pipeline |
...
- [x] `scripts/image_quality_checker.py` — validates downloaded images (Pillow: format, dimensions ≥10px, SHA256), writes `image_validated` partition + registers Glue partition
...
- [x] `infra/terraform/glue.tf` — two external Glue tables: `image_download_log`, `image_validated`
...
- [x] `infra/terraform/lambda.tf` — `image_pipeline_trigger` Lambda + S3 notification on `bronze/images/download_log/meta/**/_SUCCESS`
...
```
Check S3 for:
- `bronze/images/download_log/dt={today}/data.jsonl.gz` — status rows
- `bronze/images/download_log/meta/{today}/_SUCCESS` — trigger marker
...
**Note on first run:** `image_download_log` will be an empty Glue table with no partitions — the LEFT JOIN in the Athena query handles this correctly, returning all catalog rows with `status IS NULL`.
...
- [x] `image_catalog` confirmed as `stg_product_image_download_log` — update `--athena-table` default in `run_image_pipeline.py` to `stg_product_image_download_log`
- [x] scraper-pipeline: `stg_product_image_download_log` extended with `download_status`, `download_error_run_id`, `s3_blob_key`; `downloaded` now derived from `image_validated` presence; `stg_product_serving_latest` surfaces `s3_blob_key`
